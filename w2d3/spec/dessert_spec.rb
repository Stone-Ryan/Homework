...
describe Dessert do
  let(:chef) { double("chef", name: 'Wallace') }
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ingredients = ['chocolate', 'milk', 'eggs', 'butter', 'sugar']

      ingredients.each do |thing|
        brownie.add_ingredient(thing)
      end
  ...
  describe "#eat" do
    it "subtracts an amount from the quantity" do
      brownie.eat(6)
      expect(brownie.quantity).to eq(94)
    end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return('Chef Wallace the Great Baker')
      expect(brownie.serve).to eq('Chef Wallace the Great Baker has made 100 brownies!')
    end
  end
